...
@Component({
  ...
  templateUrl: './pendingtrfbalancereq.component.html',
  styleUrls: ['./pendingtrfbalancereq.component.css']
})
export class PendingtrfbalancereqComponent implements OnInit {
  ...
  rejectTrfbalancereq(idx, trfbalancereqid, label): void {
    ...
    if(confirm('Do you really want to reject this transfer from: ' + label + ' record?')){
        ...
        .subscribe(data => {
          if (data.success === false) {
            ...
            this.loading = false;
            this.trfbalancereqs.splice(idx, 1);
